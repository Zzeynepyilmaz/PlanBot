...
class WeatherRepository(private val apiKey: String) {
    ...
    suspend fun getCurrentWeatherByLocation(lat: Double, lon: Double): String = withContext(Dispatchers.IO) {
        try {
            val url =
                "https://api.openweathermap.org/data/2.5/weather?lat=$lat&lon=$lon&appid=$apiKey&lang=tr"
            val response = URL(url).readText()
            val json = JSONObject(response)
            val weather = json.getJSONArray("weather")
                .getJSONObject(0)
